...
Examples:
    ...
    # Copy/replace index.html to log directory, always injecting current API key
    ...
    if os.path.exists(index_src):
        ...
        content = content.replace('YOUR_GOOGLE_MAPS_API_KEY', api_key)

        # Write modified version to logs directory (always overwrite)
        with open(index_dst, 'w', encoding='utf-8') as f:
            f.write(content)

        print(f"Injected API key into index.html in {log_dir} (key: {api_key[:10]}...)")
