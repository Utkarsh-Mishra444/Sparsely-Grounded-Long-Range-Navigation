...
git clone https://github.com/Utkarsh-Mishra444/Sparsely-Grounded-Long-Range-Navigation.git
cd Sparsely-Grounded-Long-Range-Navigation
...
export GEMINI_API_KEY="your-gemini-key"  # or OPENAI_API_KEY, etc.
...
```yaml
# Dataset (provide your own paths JSON; not included in this repo)
paths_file: "/path/to/paths.json"
...
# Strategy
strategy_module: "strategies.agent_nav"  # AgentNav
...
│   ├── agent_nav.py        # AgentNav (VoP + Memory)
...
│   ├── path_generator/     # Web UI for generating navigation paths
├── data/
...
A web-based tool for generating and managing navigation paths in Google Street View.

![Path Generator Interface](tools/path_generator/docs/screenshots/main_interface_1.png)

Interactive Street View with Google Maps integration, landmark markers, and polygon management for defining destination areas.
...
Open `http://localhost:8766` to access the interface.

**Features:**
- Interactive Street View navigation with click-to-explore
- Pathfinding with DFS, temperature annealing, and direction bias
- Bulk path generation with configurable parameters
- Landmark and polygon management for all 4 cities

See [`tools/path_generator/README.md`](tools/path_generator/README.md) for full documentation.

### Log Viewer

Visualize navigation experiments on satellite maps with path tracing and decision point markers.

![Log Viewer](tools/path_generator/docs/screenshots/log_viewer_1.png)

Gold markers indicate API call decision points where the agent made navigation choices. Blue markers show intermediate positions along the route.

**Features:**
- Satellite map visualization of navigation paths
- Decision point inspection (API calls vs. regular steps)
- Success/failure status tracking
- Multi-experiment browsing and comparison
