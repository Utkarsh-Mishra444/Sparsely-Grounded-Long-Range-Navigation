"""
Looptrap variant of the memory strategy for Street View navigation.

This strategy includes escalating warnings when the agent repeatedly chooses
the same direction at an intersection, helping break out of navigation loops.

Key features:
- Escalating per-direction warnings (>3, >5, >10 thresholds)
- Global exploration advice based on worst-case direction count
- Dead-end detection and marking
- Markovian position memory support via variant_params
"""
...
class AdvancedStreetViewStrategy(Strategy):
    ...
    This looptrap variant includes escalating warnings to break navigation loops.
    """
    ...
    def __init__(self,
        ...
        # Extract variant-specific params from variant_params dict
        variant_params = variant_params or {}
        # Grid parameters for self-positioning agent (8-image variant: 8x3 grid, middle row)
        self.grid_params = variant_params.get('grid_params', {"X": 8, "Y": 3})
        self.grid_selection = variant_params.get('grid_selection', "middle_row")
        # Self-positioning interval (how often to perform self-positioning after first decision)
        self.SELF_POSITION_INTERVAL = variant_params.get('self_position_interval', self.SELF_POSITION_INTERVAL)
        # Store variant_params for agent.py to access
    ...
    def decide_with_vision_api(self, agent: Any, links: List[Dict], log: bool = True) -> str:
        ...
        if not images:
            ...
            # Clear stale dead-end detection state to prevent false positives
            # Without images, we can't visually identify intersections, so dead-end detection should be skipped
            if hasattr(self, 'last_intersection_hash'):
                delattr(self, 'last_intersection_hash')
            if hasattr(self, 'last_chosen_index'):
                delattr(self, 'last_chosen_index')
    ...
    def fetch_images(self, pano_id: str, links: List[Dict]) -> Tuple[List[Dict], List[str]]:
        ...
        NOTE: This version does NOT use image caching - images are fetched
        directly from the Google Street View API on each call.
    ...
    def prepare_prompt(self, agent: Any, num_images: int, cardinal_directions: List[str], headings: List[float] | None = None,
        ...
        # ── COMPONENT 7: Previous visits information with LOOPTRAP ESCALATION ───────────────────
        # This component provides detailed information about previous visits to this exact intersection
        # Includes visit count, previous direction choices, dead end detection, and ESCALATING exploration guidance
        ...
        if self._pc('include_previous_visits') and previous_visits:
            ...
            # WITH ESCALATING WARNINGS based on per-direction count
            decision_counts: list[str] = []
            max_any_direction_count = 0
            ...
            for idx_str, data in decisions.items():
                ...
                max_any_direction_count = max(max_any_direction_count, count)
                idx = int(idx_str)
                if idx < len(unique_option_ids):
                    direction = cardinal_directions[idx]
                    option_id = unique_option_ids[idx]

                    # LOOPTRAP: Escalating per-direction suffix based on how many times THIS direction was chosen
                    if count > 10:
                        suffix = " - THIS IS NOT WORKING, choose a different direction"
                    elif count > 5:
                        suffix = " - probably not the right way, try something else"
                    elif count > 3:
                        suffix = " - consider trying other directions"
                    else:
                        suffix = ""

                    decision_counts.append(
                        f"{direction} (Option {option_id}): chosen {count} times{suffix}")
            ...
            # LOOPTRAP: Escalating exploration advice based on worst-case direction count
            if max_any_direction_count > 10:
                exploration_advice = (
                    "You have repeatedly chosen the same direction without making progress. "
                    "That approach is not working. Choose a different direction now."
                )
            elif max_any_direction_count > 5:
                exploration_advice = (
                    "Some directions have been tried many times without success. "
                    "Strongly consider trying a less-attempted or untried direction."
                )
            else:
                exploration_advice = (
                    "You should try to explore new directions that haven't been taken before or that aren't dead ends, "
                    "unless there's a compelling reason to revisit a previous path."
                )
        ...
        prompt = (
            ...
            f"Analyze the images to determine the best direction towards {destination}. "
