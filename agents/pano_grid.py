...
def build_fixed_grid_urls(
    ...
    *, size: tuple[int, int] = (512, 512),
    equal_area: bool = False,
    yaw0_deg: float = 0.0,
    grid_selection: str | list[int] | None = None
) -> list[str]:
    """
    Return N = X*Y URLs (or subset if grid_selection is specified), each centered on a tile center
    with a single FOV so tiles cover at least their intended angular extents.
    Works with base_url that already has either `pano=` or `location=lat,lng`.

    Args:
        base_url: Base Street View API URL
        X: Number of columns (horizontal tiles)
        Y: Number of rows (vertical tiles)
        Tup_deg: Trim angle from top (degrees)
        Tdown_deg: Trim angle from bottom (degrees)
        size: Tile size (width, height)
        equal_area: Use equal-area rows instead of equal-angle
        yaw0_deg: Initial yaw rotation (0° = North)
        grid_selection: Optional selection pattern (see apply_grid_selection)

    Returns:
        List of URLs (all X*Y URLs if grid_selection is None, or selected subset)
    """
    W, H = size
    ...
    # Apply grid selection if specified
    if grid_selection is not None:
        urls = apply_grid_selection(urls, X, Y, grid_selection)
...
def apply_grid_selection(
    ...
    X: int,
    Y: int,
    selection: str | list[int] = "all"
) -> list:
    """
    Apply grid selection pattern to select subset of items from grid.
    Grid is built row-major: row 0 = indices [0..X-1], row 1 = [X..2X-1], etc.
    
    Args:
        items: Full list of items from grid (X*Y total)
        X: Number of columns in grid
        Y: Number of rows in grid
        selection: Selection pattern:
            - "all": Use all items (default)
            - "middle_row": Select middle row
            - "first_row": Select first row
            - "last_row": Select last row
            - List of indices: Custom selection by indices
    
    Returns:
        Selected subset of items based on selection pattern
    """
    total_items = len(items)
    if total_items != X * Y:
        # Size mismatch - return all items with warning
        return items
    
    if selection == "all":
        return items
    elif selection == "middle_row":
        # Select middle row (row at index Y//2)
        row_idx = Y // 2
        start_idx = row_idx * X
        end_idx = start_idx + X
        return items[start_idx:end_idx]
    elif selection == "first_row":
        # Select first row
        return items[0:X]
    elif selection == "last_row":
        # Select last row
        start_idx = (Y - 1) * X
        return items[start_idx:start_idx + X]
    elif isinstance(selection, list):
        # Custom selection by indices
        selected = []
        for idx in selection:
            if 0 <= idx < total_items:
                selected.append(items[idx])
        return selected if selected else items  # Fallback to all if no valid indices
    else:
        # Unknown pattern - return all items
        return items
