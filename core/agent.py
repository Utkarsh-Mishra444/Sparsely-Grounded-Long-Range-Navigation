...
class StreetViewAgent(Agent):
    ...
    def __init__(self,
        ...
        # Get variant_params from strategy instance (passed through from config)
        variant_params = getattr(strategy_instance, "variant_params", {})

        # Get grid_params and grid_selection from strategy instance (if set)
        grid_params = getattr(strategy_instance, "grid_params", None)
        grid_selection = getattr(strategy_instance, "grid_selection", None)

        # Initialize self-positioning agent with Markovian memory
        memory_size = variant_params.get('self_pos_memory_size', 5)  # Default to 5 past estimates
